    ...
    <div className="p-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
        <div>
          ...
          <p className="text-gray-400 mt-1">Showcase your best work to potential clients</p>
        </div>
        <Button 
          onClick={() => {
            setCurrentProject({ title: "", description: "", image: "", category: "Web App", tags: "" });
      ...
      {isLoading && projects.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project) => (
            <Card key={project.id} variant="glass" className="group overflow-hidden border-white/5">
              <div className="aspect-video relative overflow-hidden">
                ...
                <div className="absolute top-3 left-3">
                  <span className="px-3 py-1 rounded-full bg-black/60 backdrop-blur-md text-[10px] font-black uppercase tracking-widest text-blue-400 border border-blue-500/20">
                    {project.category}
                  </span>
                </div>
              </div>
              <div className="p-6">
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {project.tags.split(',').map(tag => (
                    <span key={tag} className="text-[10px] text-gray-500 bg-white/5 px-2 py-0.5 rounded-md">#{tag.trim()}</span>
                  ))}
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="flex gap-4 pt-4">
                    <Button
                      ...
                      className="flex-1 py-6 rounded-xl font-bold text-gray-400 hover:text-white"
                    >
                      Cancel
                    </Button>
                    <Button
                      type="submit"
                      disabled={isLoading}
                      className="flex-1 bg-gradient-to-r from-blue-600 to-emerald-600 text-white font-bold py-6 rounded-xl shadow-lg shadow-blue-500/20"
                    >
                      {isLoading ? "Saving..." : (
                        <div className="flex items-center gap-2">
